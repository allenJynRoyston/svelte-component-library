...
<TwoSlot showLeft showRight>
  ...
  <div slot='right' style='display: flex; gap: 10px'>
    ...
    <Button  type={theme === 'dark' ? 'white' : 'black'} rounded useToggle toggled={outline} hollow={!outline} nomargin onClick={() => {outline = !outline}} >Outline</Button>
  </div>
</TwoSlot>
<hr>
...
<CodeBlock open title='Example:' snippet={`
  <Accordian ${rounded ? 'rounded' : ''} ${outline ? 'outline' : ''}>
    <div slot='title'>
      <h3>Accordion</h3>
    </div>
